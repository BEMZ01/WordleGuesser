...
def game(player, correct_word=None):
    ...
    while guesses != 6:
        ...
        print("AI suggestion 1st: " + ai_result[0])
        print(len(ai_result))
        if len(ai_result) > 2:
            print("AI suggestion 2nd: " + ai_result[1])
            print("AI suggestion 3rd: " + ai_result[2])
        if player:
            inp = input("\n:> ").upper()
...
def mainmenu():
    menu = input("Main Menu.\n1. Human\n2. Emulate\n3. Chose your own word\n\n>: ")
    while 1:
        if menu.isnumeric() and menu in ["1", "2", "3"]:
            menu = int(menu)
            break
        else:
            menu = input("Main Menu.\n1. Human\n2. Emulate\n\n>: ")
    ...
    if menu == 2:
        ...
        for x in range(int(emulate)):
            ...
            try:
                ...
                sys.stdout = sys.__stdout__
                print("Exception Raised! " + str(ExceptionError) + " \\\\ Skipping!")
    if menu == 3:
        word_choice = input("Answer word \n>: ").upper()
        while 1:
            if len(word_choice) != 5:
                word_choice = input("Answer word\n>: ").upper()
            else:
                break
        results.append(game(True, word_choice))
    won = 0
    lost = 0
    score = 0
